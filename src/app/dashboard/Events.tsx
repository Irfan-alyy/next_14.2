          ...
          {alerts.map((alert) => (
            ...
            >
              <div className="flex justify-between">
                <span className="font-mono text-sm">#{alert?.id?.slice(-5)}</span>
                <span className="text-xs text-gray-600">
                  {new Date(alert.event_time).toLocaleDateString()}
                </span>
              </div>
              <p className="text-sm mt-1 text-blue-800">{eventMap[alert.event_type]}</p>
            </li>
          ))}
        </ul>
      )}
...
export function OrdersAlertSkeleton(){
    return(
        <ul className="space-y-3">
         <li
              className="p-3 py-8 border rounded bg-blue-50 hover:bg-blue-100 transition"
            >
              <div className="flex justify-between">
                <span className="font-mono text-sm bg-gray-600 px-4">   </span>
                <span className="text-xs text-gray-600 px-4 bg-gray-600">
